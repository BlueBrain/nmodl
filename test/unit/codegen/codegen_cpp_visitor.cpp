...
std::shared_ptr<CodegenCorenrnCppVisitor> create_corenrn_cpp_visitor(const std::shared_ptr<ast::Program>& ast,
    ...
    auto cv = std::make_shared<CodegenCorenrnCppVisitor>("temp.mod", ss, "double", false);
    cv->setup(*ast);
    return cv;
}
...
std::string get_instance_var_setup_function(std::string& nmodl_text) {
    ...
    std::stringstream ss;
    auto cvisitor = create_corenrn_cpp_visitor(ast, nmodl_text, ss);
...
std::string get_instance_structure(std::string nmodl_text) {
    ...
    // setup codegen
    std::stringstream ss{};
    CodegenCorenrnCppVisitor cv{"temp.mod", ss, "double", false};
    cv.setup(*ast);
