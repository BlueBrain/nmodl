    ...
    GIVEN("A empty property") {
        ...
        WHEN("added multiple properties") {
            ...
            THEN("to_string returns all added properties") {
                REQUIRE_THAT(to_string(result), ContainsSubstring("local"));
                REQUIRE_THAT(to_string(result), ContainsSubstring("global"));
                REQUIRE_THAT(to_string(result), ContainsSubstring("write_ion"));
            }
            WHEN("checked for property") {
    ...
    GIVEN("A global SymbolTable") {
        ...
        WHEN("checked methods and member variables") {
            THEN("all members are initialized") {
                ...
                REQUIRE_THAT(table->name(), ContainsSubstring("Na"));
                REQUIRE_THAT(table->get_parent_table_name(), ContainsSubstring("None"));
                REQUIRE_THAT(table->position(), ContainsSubstring("UNKNOWN"));
            }
        }
        WHEN("insert symbol") {
            table->insert(symbol);
            ...
            WHEN("re-inserting the same symbol") {
                THEN("throws an exception") {
                    REQUIRE_THROWS_WITH(table->insert(symbol), ContainsSubstring("re-insert"));
                }
            }
        ...
        WHEN("added same symbol with existing property") {
            ...
            THEN("throws an exception") {
                REQUIRE_THROWS_WITH(mod_symtab.insert(symbol3),
                                    ContainsSubstring("Re-declaration"));
            }
        }
