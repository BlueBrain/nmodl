...
void CodegenIspcVisitor::print_global_function_common_code(BlockType type) {
    ...
    printer->add_line("{}int* {}node_index = ml->nodeindices;"_format(k_const(), ptr_type_qualifier()));
    printer->add_line("double* {}data = ml->data;"_format(ptr_type_qualifier()));
    printer->add_line("{}double* {}voltage = nt->_actual_v;"_format(k_const(), ptr_type_qualifier()));
...
void CodegenIspcVisitor::print_compute_functions() {
    ...
    for (const auto& function: info.functions) {
        if (!program_symtab->lookup(
                function->get_node_name()).get()->has_all_status(Status::inlined)) {
...
void CodegenIspcVisitor::print_mechanism_range_var_structure() {
    ...
    for (auto& var: codegen_int_variables) {
        ...
        if (var.is_index || var.is_integer) {
            ...
            printer->add_line("{}{}* {}{};"_format(qualifier, int_type, ptr_type_qualifier(), name));
        } else {
            auto qualifier = var.is_constant ? k_const() : "";
...
void CodegenIspcVisitor::print_ispc_helper_ds() {
    ...
    printer->add_line("{}{}* {}{};"_format("const uniform ", int_type, ptr_type_qualifier(), "node_index"));
    printer->add_line("{}* {}{};"_format(float_type, ptr_type_qualifier(), "data"));
    printer->add_line("{}* {}{};"_format(int_type, ptr_type_qualifier(), "indexes")); // @todo: Datum, not int
