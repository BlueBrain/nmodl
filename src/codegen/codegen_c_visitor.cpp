...
void CodegenCVisitor::print_mechanism_range_var_structure(bool print_initialisers) {
    ...
    for (auto& var: codegen_float_variables) {
        ...
        printer->fmt_line("{}{}* __restrict__ {}{};", qualifier, type, name, value_initialise);
    }
    for (auto& var: codegen_int_variables) {
        ...
        if (var.is_index || var.is_integer) {
            ...
            printer->fmt_line(
                "{}{}* __restrict__ {}{};", qualifier, int_type, name, value_initialise);
        } else {
            auto qualifier = var.is_constant ? "const " : "";
...
void CodegenCVisitor::print_global_function_common_code(BlockType type,
    ...
    printer->add_line("int pnodecount = ml->_nodecount_padded;");
    printer->add_line("const int* __restrict__ node_index = ml->nodeindices;");
    printer->add_line("double* __restrict__ data = ml->data;");
    printer->add_line("const double* __restrict__ voltage = nt->_actual_v;");

    if (type == BlockType::Equation) {
        printer->add_line("double* __restrict__ vec_rhs = nt->_actual_rhs;");
        printer->add_line("double* __restrict__ vec_d = nt->_actual_d;");
        print_rhs_d_shadow_variables();
    }
    printer->add_line("Datum* __restrict__ indexes = ml->pdata;");
    printer->add_line("ThreadDatum* __restrict__ thread = ml->_thread;");

    if (type == BlockType::Initial) {
        printer->add_newline();
        printer->add_line("setup_instance(nt, ml);");
    }
    printer->fmt_line("auto* const __restrict__ inst = static_cast<{}*>(ml->instance);",
                      instance_struct());
    printer->add_newline(1);
}
...
void CodegenCVisitor::print_net_receive_buffering(bool need_mech_inst) {
    ...
    printer->add_line("NetReceiveBuffer_t* __restrict__ nrb = ml->_net_receive_buffer;");
    if (need_mech_inst) {
        printer->fmt_line("auto* const inst = static_cast<{0}*>(ml->instance);", instance_struct());
    }
