...
void LocalToRangeVisitor::visit_program(ast::Program &node) {
    ...
    for (auto &top_level_node : top_level_nodes) {
        std::cout << top_level_node->get_node_type_name() << std::endl;
        if (top_level_node->is_local_list_statement()) {
            ...
            for (auto& local_variable : local_variables) {
                ...
                if (symbol_table->lookup(variable_name)->get_write_count() > 0) {
                    auto string = std::make_shared<ast::String>(local_variable->get_node_name());
                    auto ast_name = ast::Name(string);
                    auto name = std::make_shared<ast::Name>(ast_name);
                    range_variables.emplace_back(new ast::RangeVar(name));
                    local_variables_to_remove.emplace(local_variable.get());
                }
            }

            /// remove offending local variables
            std::static_pointer_cast<ast::LocalListStatement>(top_level_node)->erase_local_var(
                    local_variables_to_remove);
    ...
    if (!range_variables.empty()) {
        ...
        std::static_pointer_cast<ast::NeuronBlock>(*neuron_block)->get_statement_block()->emplace_back_statement(range_statement);
    }
}
