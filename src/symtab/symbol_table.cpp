...
SymbolTable::SymbolTable(const SymbolTable& table) : symtab_name{table.name()}, global{table.global_scope()}, node{nullptr}, parent{nullptr} {}
...
std::vector<std::shared_ptr<Symbol>> SymbolTable::get_variables(NmodlType with, NmodlType without) const {
